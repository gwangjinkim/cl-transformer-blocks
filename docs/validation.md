...
The expanded worker gate dynamically selects `AutoModelForCausalLM`, forwards rank-three FP32 `inputs_embeds`, and performs three stateful clipped AdamW steps under Lisp control. Individual CPU losses use a `1e-6` absolute gate: independent Torch processes on the hosted x86 runner differed by two FP32 representable steps (`4.76837158203125e-7`) at a loss near 3.3. The MPS loss gate remains `3e-5`. Trained logits differed from the independent three-step CPU Python reference by at most 8.94e-8 on CPU and 2.43e-6 on MPS. New CPU/MPS workers reloaded their contributed checkpoints and reproduced the corresponding pre-export logits exactly; standalone Python processes checked the trained parameters and logits against the independent reference. The same expanded gate is required by the pending CUDA workflow.
